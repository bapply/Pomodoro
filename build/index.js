...
/***/ function(module, exports, __webpack_require__) {
	...
	var hz = new _client2.default();
	var Pomodoro = hz('pomodoro');
	...
	var $timer = (0, _jquery2.default)('#timer');
	var $start = (0, _jquery2.default)('#start');
	var $pause = (0, _jquery2.default)('#pause');
	var $stop = (0, _jquery2.default)('#stop');
	var $count = (0, _jquery2.default)('#count');
	...
	function processModel(model) {
	  if (timer.active) state = 'active';else if (relax.active) state = 'relax';else if (timer.pause) state = 'pause';else state = 'stop';
	
	  $timer.text(format($timer.data(state), model.time));
	  $count.text($count.data('always').replace('%count', timer.count));
	  $start.text($start.data(state)).toggle(shouldDisplay($start));
	  $pause.text($pause.data(state)).toggle(shouldDisplay($pause));
	  $stop.text($stop.data(state)).toggle(shouldDisplay($stop));
	  console.log(model);
	}
	...
	function format(f, time) {
	  ...
	  return f.replace('%m', minutes).replace('%s', seconds);
	}
